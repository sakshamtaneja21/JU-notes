/*
NOTE 1: The O(sqrt(n)) can also be used for this purpose but it will depend upon the constraints. check while using 

Note 2: The idea works well for n upto the order of 10^7 only. Beyond this we will face memory issues.
...
Time Complexity -
    ...
    Therefore will have log n division steps. Hence, We can say that our Time Complexity will be O(log n) in worst case for each query. Thus 
    for Q queries we will have complexity of O(Q * log n). And the total complexity will be -
			 O(Q * log n (for Q queries of number n) + n log log n (for seive))
